...
C = 1/math.sqrt(2) # an experienced factor
...
def best_child(node):
    ...
    for child in node.children:
        ...
        #if score > best_score:
            #best_chi, best_score = [child], score
        #if score == best_score:
            #best_chi.append(child)
        if score > best_score:
            best_chi, best_score = child, score        
    # return random.choice(best_chi)
    return best_chi
...
def default_policy(state):
    ...
    return -1 if state._has_liberties(current_state.player()) else 1
